...
export async function updateKitchenOrderItemStatus(orderItemId, status) {
  try {
    const response = await ApiClient.post(
      `/orders/update-status/${orderItemId}`,
      {
        status,
      }
    );
 ...
 *  */
export async function cancelKitchenOrder(orderIds) {
  try {
 ...
 * @param {Array<Number>} orderIds - list of orders to cancel
 *  */
export async function completeKitchenOrder(orderIds) {
  try {
